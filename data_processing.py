...
workdir = "/Users/christopherhempel/Google Drive/PhD UoG/ExStream project/pipeline_results/"
## Full path to output dorectory in which the resulting .csv file is saved
outdir = "/Users/christopherhempel/Google Drive/PhD UoG/ExStream project/data_analysis/"
...
for file in sample_files:
    ...
    df = df.rename(columns={df.columns[0]: 'sequence_name'}).replace("Unknown", "NA")\
    ...
    df_small = df[used_ranks + ["counts", "sequence_length"]]                                               ## Adapt in future
    ...
    if df.empty:
        ...
        #df_agg = df_small.groupby(list(df_small.columns)[:-2])["covered_bases", "sequence_length"].sum().reset_index()             ## Adapt in future
        df_agg = df_small.groupby(list(df_small.columns)[:-2])["counts", "sequence_length"].sum().reset_index()
        #### Determine average per-base coverage for each taxon
        #df_agg["per_base_coverage"] = df_agg["covered_bases"]/df_agg["sequence_length"]                                            ## Adapt in future
        #df_agg=df_agg.drop(["sequence_length", "covered_bases"], axis=1)
        df_agg=df_agg.drop(["sequence_length"], axis=1)
        #### Turn coverages into relative abundances:
        #df_agg["per_base_coverage"]=df_agg["per_base_coverage"]/df_agg["per_base_coverage"].sum()                                           ## Adapt in future
        df_agg["counts"]=df_agg["counts"]/df_agg["counts"].sum()
    ### Rename counts col
    #df_agg.rename(columns = {'per_base_coverage':'abundance'}, inplace = True)                                           ## Adapt in future
    df_agg.rename(columns = {'counts':'abundance'}, inplace = True)
...
sample_reads=pd.read_table(readnums, header=None, index_col=0, squeeze=True).to_dict()
...
## TO DO: What to do with NaN: seems that some pipelines didn't work for our test                                               ## Adapt in future
## data, let' see how the new pipeline runs work and see if this issue persists
## For now: drop columns with NaN
