    ...
    for dtype in dtypes:
        for fselect in fselects:
            ...
            modeldf = df[
                (df["model"] == model)
                & (df["datatype"] == dtype)
                & (df["feature-selection"] == fselect)
            ]
            modeldf = modeldf.sort_values(
                by=["rank"], key=lambda x: x.map(rank_sort_dic)
            )
            fig = px.line(
                modeldf,
                x="rank",
...
for seqtype in seqtypes:
    for dtype in dtypes:
        for fselect in fselects:
            ...
            seqtypedf = seqtypedf.sort_values(
                by=["rank"], key=lambda x: x.map(rank_sort_dic)
            )
            fig = px.line(
                seqtypedf,
...
for seqtype in seqtypes:
    for dtype in dtypes:
        for model in models:
            ...
            fselectdf = fselectdf.sort_values(
                by=["rank"], key=lambda x: x.map(rank_sort_dic)
            )
            fig = px.line(
                fselectdf,
...
heatmap_df = (
    df.replace("its-otu", "a-its-otu")
    .replace("its-esv", "b-its-esv")
    .replace("16s-otu", "c-16s-otu")
    .replace("16s-esv", "d-16s-esv")
    .replace("16s-its-otu", "e-16s-its-otu")
    .replace("16s-its-esv", "f-16s-its-esv")
)
heatmap_df = (
    heatmap_df.replace("phylum", "a-phylum")
    .replace("class", "b-class")
    .replace("order", "c-order")
    .replace("family", "d-family")
    .replace("genus", "e-genus")
    .replace("species", "f-species")
)
...
## Visualization
### Category 1
df_wide = heatmap_df.pivot_table(
    index="rank+model", columns="stype+fs", values="test_mcc_mean"
).transpose()
...
### Category 2
df_wide = heatmap_df.pivot_table(
    index="stype+rank", columns="fs+model", values="test_mcc_mean"
).transpose()
heatmap = px.imshow(df_wide)
heatmap.update_xaxes(tickangle=90)
heatmap.show()
...
for col in X_dummies.columns:
    ...
    cor_dic[col] = cor
cor_df_no_clust = (
    pd.DataFrame(cor_dic, index=["coefficient", "p-value"]).transpose().reset_index()
)
...
cor_df = (
    pd.concat([cor_df_no_clust, cor_df_w_clust]).reset_index().drop(["level_0"], axis=1)
)
...
cor_df = cor_df.reindex(
    [
        "seqmethod_its",
        "seqmethod_16s",
        "seqmethod_16s-its",
        "seqmethod_metagenomics",
        "seqmethod_totalrnaseq",
        "clustering_otu",
        "clustering_esv",
        "rank_phylum",
        "rank_class",
        ...
        "datatype_abundance",
        "datatype_pa",
        "feature-selection_w-fs",
        "feature-selection_wo-fs",
        "model_knn",
        "model_lor-lasso",
        "model_lor-ridge",
        "model_lsvc",
...
for seqtype in seqtypes:
    ...
    for col in X_dummies.columns:
        ...
        cor_dic[col] = cor
    cor_df = (
        pd.DataFrame(cor_dic, index=["coefficient", "p-value"])
        .transpose()
        .reset_index()
    )
