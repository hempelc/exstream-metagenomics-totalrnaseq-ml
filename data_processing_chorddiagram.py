...
indir = "/Users/christopherhempel/Google Drive/PhD UoG/ExStream project/data_files_for_ml"
...
## Files for counting
nodes_df = pd.read_csv(os.path.join(indir, nodes_dmp_file), sep="\t", usecols=[0,8], names=["tax_id", "division_id"], header=None, dtype=str)
division_df = pd.read_csv(os.path.join(indir, division_dmp_file), sep="\t", header=None, usecols=[0,4], names=["division_id", "division"], dtype=str)
tax_id_division_df = pd.merge(left=nodes_df, right=division_df, on="division_id").drop("division_id", axis=1)
    ...
    for seqtype_chord in seqtypes_chord:
        ...
        for taxon in taxa_list_dict[rank][seqtype_chord]:
            if taxon.lower() in NCBI_scientific_dict:
                ...
                try:
                    translated_taxon = key_from_value(NCBI_scientific_dict, NCBI_non_scientific_dict[taxon.lower()]).lower()
                except:
        ...
        taxon_dict = {k:v for k,v in taxon_dic.items() if 'candidatus' not in k if 'candidate' not in k if '[candida]' not in k}
...
for rank in ["phylum", "genus", "species"]:
    for seqtype_chord in taxa_list_dict[rank].keys():
        ...
        taxid_df_tmp = pd.DataFrame(taxa_list_dict[rank][seqtype_chord], index=["tax_id"]).transpose().reset_index().drop("index", axis=1)
        tax_id_division_df_tmp = pd.merge(left=taxid_df_tmp, right=tax_id_division_df, on="tax_id", how="left")
        ## Save rows with NA
        no_ids_df_tmp = tax_id_division_df_tmp[tax_id_division_df_tmp["division"].isna()]
        no_ids_df.loc["{}_{}".format(rank, seqtype_chord)] = [rank, seqtype_chord, list(no_ids_df_tmp["tax_id"])]
...
## Phylum
taxa_list_dict["phylum"]["otu_its_taxa"] = {**taxa_list_dict["phylum"]["otu_its_taxa"], "monoblepharidomycetes": "451454", "basidiobolomycetes": "1399768"}
taxa_list_dict["phylum"]["esv_its_taxa"]["monoblepharidomycetes"] = "451454"
taxa_list_dict["phylum"]["esv_16s_taxa"] = {**taxa_list_dict["phylum"]["esv_16s_taxa"], "deltaproteobacteria": "28221", 'thermodesulfobacteria': "200940"}
...
taxa_list_dict["genus"]["esv_its_taxa"] = {**taxa_list_dict["genus"]["esv_its_taxa"], "orbilia": "47022",
    "chordomyces": "1654916", "teunia": "2724800", "helicodendron": "319381",
    "hongkongmyces": "1545378", "articulospora": "253308", 'monocillium': "581188",
    "neosetophoma": "798068", 'papiliotrema': "189450", "aspergillus": "5052",
    "phomatospora": "189357", "pseudohalonectria": "40655", "thelephora": "56489"}
taxa_list_dict["genus"]["otu_its_taxa"] = {**taxa_list_dict["genus"]["otu_its_taxa"], "orbilia": "47022", 'blastobotrys': "43971",
    'chalara': "13000", "chordomyces": "1654916", "teunia": "2724800",
    'dactylaria': "47274", 'gyrophanopsis': "2789184", "helicodendron": "319381",
    "hongkongmyces": "1545378", 'monocillium': "581188",
    'alnicola': "91662", 'neoconiothyrium': "2093505", "neosetophoma": "798068",
    "aspergillus": "5052", "phomatospora": "189357", 'remleria': "1776023",
    "thelephora": "56489", "graphium": "1503932", "ellisembia": "375389"}
...
taxa_list_dict["species"]["esv_its_taxa"] = {**taxa_list_dict["species"]["esv_its_taxa"], "orbilia oligospora": "2813651",
    "chordomyces antarcticum": "1525356", "teunia cuniculi": "365330", "helicodendron westerdijkae": "409251",
    "hongkongmyces thailandica": "2059664", "articulospora tetracladia": "253309", 'monocillium sp. \'griseo-ochraceum\'': "2075169",
    "neosetophoma samarorum": "798069", 'papiliotrema sp. \'fusca\'': "192649", "aspergillus penicillioides": "41959",
    "phomatospora dinemasporium": "2718578", "pseudohalonectria adversaria": "1953033", "thelephora alnii": "56491", "niesslia mucida": "3029106"}
taxa_list_dict["species"]["otu_its_taxa"] = {**taxa_list_dict["species"]["otu_its_taxa"], "orbilia oligospora": "2813651", 'blastobotrys niveus': "44073",
    'chalara heteroderae': "2981829", 'chordomyces antarcticum': "1525356", 'teunia cuniculi': "365330",
    'dactylaria mitrata': "228885", 'gyrophanopsis polonensis': "2789189", 'helicodendron westerdijkae': "409251",
    'hongkongmyces thailandica': "2059664", 'monocillium sp. \'griseo-ochraceum\'': "2075169",
    'alnicola alnetorum': "242214", 'neoconiothyrium hakeae': "2093506", "neosetophoma samarorum": "798069",
    "aspergillus penicillioides": "41959", "phomatospora dinemasporium": "2718578", 'remleria tetraspora': "1776023",
    "thelephora alnii": "56491", "niesslia mucida": "3029106"}
...
for rank in ["phylum", "genus", "species"]:
    for seqtype_chord in taxa_list_dict[rank].keys():
        ...
        tax_id_division_df_tmp = tax_id_division_df_tmp[~tax_id_division_df_tmp["division"].isna()]
        ...
        for div in division_df["division"]:
            ...
            division_count_df.loc["{}_{}_{}".format(rank, seqtype_chord, div)] = [rank, seqtype_chord, div, counter]
division_count_df.to_csv(os.path.join(indir, "division_counts.csv".format(rank)), index=False)
...
for rank in ["phylum", "genus", "species"]:
    ...
    for seqtype_chord in seqtypes_chord:
        ...
        unique_taxa = len([x for x in taxa_list_dict[rank][seqtype_chord] if x not in other_taxa])
        ...
        overlap_sum = overlap_df[overlap_df.apply(lambda r: r.str.contains(seqtype_chord, case=False).any(), axis=1)]["overlap"].sum()
        size_factor = overlap_sum/nonunique_taxa
        unique_taxa = unique_taxa * size_factor
        # Add to overlap df unique
        overlap_df_unique.loc[len(overlap_df_unique)] = [seqtype_chord, seqtype_chord, unique_taxa]

    ## Organize df
    overlap_df = pd.concat([overlap_df_unique, overlap_df])
    overlap_df["from"] = overlap_df["from"].str.replace("dna_taxa", "Metagenomics").str.replace("rna_taxa", "Total RNA-Seq")\
        .str.replace("esv_16s_taxa", "16S ESV").str.replace("esv_its_taxa", "ITS-2 ESV").str.replace("otu_16s_taxa", "16S OTU").str.replace("otu_its_taxa", "ITS-2 OTU")
    overlap_df["to"] = overlap_df["to"].str.replace("dna_taxa", "Metagenomics").str.replace("rna_taxa", "Total RNA-Seq")\
        .str.replace("esv_16s_taxa", "16S ESV").str.replace("esv_its_taxa", "ITS-2 ESV").str.replace("otu_16s_taxa", "16S OTU").str.replace("otu_its_taxa", "ITS-2 OTU")

    ### Save for manual processing in R
    overlap_df.to_csv(os.path.join(indir, "overlap_df_{0}.csv".format(rank)), index=False)
    ...
    if rank=="phylum":
        ...
        count_df_narrow = count_df_narrow.append({'seqtype': 'ref1', 'taxa_cat': 'unique_taxa', 'count': 100}, ignore_index = True)
        count_df_narrow = count_df_narrow.append({'seqtype': 'ref2', 'taxa_cat': 'unique_taxa', 'count': 1000}, ignore_index = True)
        count_df_narrow = count_df_narrow.append({'seqtype': 'ref1', 'taxa_cat': 'total_taxa', 'count': 2000}, ignore_index = True)
        count_df_narrow = count_df_narrow.append({'seqtype': 'ref2', 'taxa_cat': 'total_taxa', 'count': 3000}, ignore_index = True)
